...
@frappe.whitelist()
def get_my_project_names_by_status(status: str = "") -> dict:
	"""
	Smart Board "Status Projects" view helper.

	Returns the FULL list of Project names where:
	- current user is a member of Project.custom_team_members
	- Project is active (is_active = "Yes")
	- Project.status equals the given `status` (Unknown bucket matches empty status,
	  to mirror the dashboard cards generated by `get_my_projects_with_roles`)

	Why this exists:
	- The Dashboard's status cards are computed against the user's FULL related-project
	  set on the server, but `get_my_projects_with_roles` only ships a paginated page
	  to the client. Reusing that paginated page to render the Status Projects table
	  silently drops projects whose status is rare and whose name sorts past the page
	  boundary. This endpoint returns the full name list for a single status so the
	  Status Projects view can build an accurate `name IN (...)` query without
	  depending on what the dashboard happens to have cached.

	Returns: { "names": [project_name, ...], "total": int }
	"""
	_ensure_logged_in()
	user = str(frappe.session.user or "").strip()
	status_str = str(status or "").strip()
	if not status_str or not user or user == "Guest":
		return {"names": [], "total": 0}

	# Child table may be permission-guarded by parent; read only rows for current user.
	# Mirrors the relation logic in `get_my_projects_with_roles` to keep both views aligned.
	try:
		team_rows = frappe.get_all(
			"Project Team Member",
			filters={"user": user},
	...
	# Respect Project read permission and only return active Projects (matches dashboard scope).
	allowed, prows = _get_readable_project_rows(
		parents,
		fields=["name", "status"],
		active_only=True,
	...
	for n in allowed:
		...
		# Same "Unknown" bucketing the dashboard uses when computing status_counts.
		if not st:
			st = "Unknown"
		if st == status_str:
			names.append(n)
